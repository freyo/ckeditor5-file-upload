...
export default class FileUploadEditing extends Plugin {
	...
	init() {
		...
		const fileTypes = editor.config.get( 'simpleFileUpload.fileTypes' )?.map(ext => lookup(ext));
		...
		this.listenTo( editor.editing.view.document, 'clipboardInput', ( evt, data ) => {
			...
			const files = Array.from( data.dataTransfer.files ).filter( file => {
				...
				return fileTypes && fileTypes.includes( file.type );
			} );
			...
			editor.model.change( writer => {
				...
				ranges && writer.setSelection( ranges );
